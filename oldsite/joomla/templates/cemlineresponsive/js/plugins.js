      ...
      for (var i = 0; i < sticked.length; i++) {
        ...
        if (scrollTop <= etse) {
          if (s.currentTop !== null) {
            //Add the display and height to remove it
            s.stickyElement.attr("style", "display: none; height: 0")
            s.stickyElement
              .css('position', '')
              .css('top', '');
...
$(".scroll").click(function (event){
   ...
   if($('#sticker').height() == 0) {
      $('#sticker').css("height", "78px")
   }
   var dest=0;
   if($(this.hash).offset().top > $(document).height()-$(window).height()){
        dest=$(document).height() - $(window).height() - $('#sticker').height();
   }else{
        dest=$(this.hash).offset().top - $('#sticker').height();
   }
   //go to destination
